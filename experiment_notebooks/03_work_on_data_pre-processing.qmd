...
```{python}
X_train, y_train.sum(), cat_cols, ord_cols
```
...
X_processed
lr = LogisticRegression(random_state=42, penalty = None, class_weight="balanced")
lr_clf = Pipeline([
("pre_processing", preprocessing),
("logistic_regression", lr)
])

lr_us = LogisticRegression(random_state=42, penalty = None)
lr_clf_us = Pipeline([
("pre_processing", preprocessing),
("logistic_regression", lr_us)
])

lr_clf_us1 = imbPipeline([
("sampling", rus),
("pre_processing", preprocessing),
("logistic_regression", lr_us)
])
...
lr_cv_us = cross_val_score(lr_clf_us, X_resampled, y_resampled, cv = 5, scoring = "f1")
lr_cv_us

lr_cv_us1 = cross_val_score(lr_clf_us1, X_train, y_train, cv = 5, scoring = "f1")
lr_cv_us1

print(f"f1 scores with original imbalanced data: {lr_cv}")
print(f"f1 scores with undersampled balanced data: {lr_cv_us}")
print(f"f1 scores with undersampling included in pipeline: {lr_cv_us1}")
```

I have found the cause of the error. When undersampling is being used in imbPipeline, it does not seem to be working!


```{python}
y_pred_lr_us = cross_val_predict(lr_clf_us, X_resampled, y_resampled, cv = 5)
```

```{python}
print("Logistic Regression classification report for manual columns")
Markdown(classification_report(y_resampled, y_pred_lr_us))
```
